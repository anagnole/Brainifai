...
import { globalInstanceExists, readFolderConfigAt, GLOBAL_BRAINIFAI_PATH } from '../../instance/resolve.js';
  ...
  return new Command('list')
    ...
    .action(async (opts: { type?: string; query?: string }) => {
      if (!globalInstanceExists()) {
        console.log('No global instance found. Run `brainifai init --global` first.');
        return;
      }

      const globalCfg = readFolderConfigAt(GLOBAL_BRAINIFAI_PATH);
      if (globalCfg) {
        for (const inst of globalCfg.instances) {
          console.log(`● ${inst.name} (${inst.type}) — ${GLOBAL_BRAINIFAI_PATH}`);
      ...
      // Exclude global-folder instances from children list
      children = children.filter(c => c.parent !== null);
